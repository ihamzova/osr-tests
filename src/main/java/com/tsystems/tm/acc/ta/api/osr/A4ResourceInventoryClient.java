...
import static com.tsystems.tm.acc.tests.osr.a4.resource.inventory.client.invoker.GsonObjectMapper.gson;
    ...
    public A4ResourceInventoryClient() {
        client = ApiClient.api(ApiClient.Config.apiConfig().reqSpecSupplier(
                () -> new RequestSpecBuilder().setConfig(config().objectMapperConfig(objectMapperConfig().defaultObjectMapper(gson())))
                        ...
                        .setBaseUri(new GigabitUrlBuilder(A4_RESOURCE_INVENTORY_MS).buildUri())));
    }

    public A4ResourceInventoryClient(AuthTokenProvider authTokenProvider){
        client = ApiClient.api(ApiClient.Config.apiConfig().reqSpecSupplier(
                () -> RequestSpecBuilders.getDefaultWithAuth(
                        GsonObjectMapper.gson(),
                        new GigabitUrlBuilder(A4_RESOURCE_INVENTORY_MS).buildUri(),
