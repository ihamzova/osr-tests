...
import static com.tsystems.tm.acc.tests.osr.a4.dpu.commissioning.client.invoker.GsonObjectMapper.gson;
import static io.restassured.RestAssured.config;
import static io.restassured.config.ObjectMapperConfig.objectMapperConfig;

@Getter
public class A4DpuCommissioningClient implements Resetable {
    ...
    public A4DpuCommissioningClient() {
        client = ApiClient.api(ApiClient.Config.apiConfig().reqSpecSupplier(
                () -> new RequestSpecBuilder().setConfig(config().objectMapperConfig(objectMapperConfig().defaultObjectMapper(gson())))
                        ...
                        .setBaseUri(new GigabitUrlBuilder(A4_INVENTORY_IMPORTER_MS).buildUri())));
    }

    public A4DpuCommissioningClient(AuthTokenProvider authTokenProvider){
        client = ApiClient.api(ApiClient.Config.apiConfig().reqSpecSupplier(
