...
@Getter
public class A4PhysicalInventoryClient {
    private final ApiClient client;

    public A4PhysicalInventoryClient(IAccessTokenProvider accessTokenProvider) {
        client = new ApiClientBuilder<>(ApiClient.class)
                .withBaseUri(new GigabitUrlBuilder(A4_PHYSICAL_INVENTORY_MS).buildUri())
                .withAccessTokenAuth(accessTokenProvider)
                .build();
    }

    public A4PhysicalInventoryClient() {
